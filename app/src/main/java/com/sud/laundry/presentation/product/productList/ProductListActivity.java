package com.sud.laundry.presentation.product.productList;
...
import com.grocery.presentation.product.PriceBottomSheetDialogFragment;
...
public class ProductListActivity extends BaseActivity implements ProductContract.View, PaginationAdapterCallback, CartCommanContract.View {


    private static final String TAG = ProductListActivity.class.getSimpleName();
    PaginationAdapterProductList adapter;
    ...
    public void setView() {
        ...
        category_id = getIntent().getIntExtra(KeyData.CATEGORY_ID, 0);

        Log.e(TAG, "setView:  cat_id " + category_id);

        productPresenter = new ProductPresenter(this, this);
        cartCommonPresenter = new CartCommonPresenter(this, this);

        adapter = new PaginationAdapterProductList(this);
    ...
    private void loadNextPage(int currentPage) {
        ...
        productPresenter.getProductList(jsonObject, currentPage);
    ...
    public void itemClickRow(int position, ProductListItem productListItem) {
        ...
        if (position == -1) {
            new PriceBottomSheetDialogFragment(productListItem).show(getSupportFragmentManager(), "dialog");
    ...
    public void produtAddInCart(int status, ProductListItem productListItem, TextView tvQuantiy, TextView tvMinus, TextView tvPlus, AVLoadingIndicatorView avLoadingIndicatorView) {
        ...
        if (currentQuntity > -1) {
            ...
            jsonObject.addProperty("product_quantity_id", 0);
    ...
    public void ProListResponse(ProductListResponse productListResponse, int PageNo) {
        ...
        Log.e(TAG, "ProListResponse: " + productListResponse.toString() + " page  " + PageNo);
        hideErrorView();

        if (productListResponse.getStatus() == 200) {

            if (PageNo == 1) {
                TOTAL_PAGES = (productListResponse.getTotalProducts() / 10) + 1;
                progressBar.setVisibility(View.GONE);
                adapter.addAll(productListResponse.getProductList());
